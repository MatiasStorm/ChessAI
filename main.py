...
class Game():
    def __init__(self):
        ...
        self.screen = pg.display.set_mode(
            (WIDTH + TF_WIDTH, HEIGHT), pg.SRCALPHA, 32)
    ...
    def handle_computer_turn(self):
        ...
        text = "Computer moved " + CHAR_TO_STR[piece[1]] + " to " + BOARD_CANVAS_CHARS[move[1][1]] + str(
            7 - move[1][0] + 1) + ". Score: " + str(move[2]) + ". Nodes: " + str(move[3][0]) + " CutOffs: " + str(move[3][1])
        self.text_panel.write(text)

    def animate_movement(self, start_tile, end_tile, piece):
        current_pos = vector(
            start_tile[1] * TILESIZE + MARGIN, start_tile[0] * TILESIZE + MARGIN)
        end_pos = vector(end_tile[1] * TILESIZE +
                         MARGIN, end_tile[0] * TILESIZE + MARGIN)
    ...
    def check_mate(self, color):
        if self.engine.check(self.piece_matrix, color):
            if self.engine.check_mate(self.piece_matrix, color):
                self.text_panel.write(
                    "Check-mate!", RED if color == 'w' else GREEN)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                if mouse_pos[0] > MARGIN and mouse_pos[0] < WIDTH - MARGIN and mouse_pos[1] > MARGIN and mouse_pos[1] < HEIGHT - MARGIN and not self.computer_turn:
                    if self.start_pos:
                        self.move_piece(
                            self.start_pos, self.get_mouse_pos_rc(), 'P')
                        self.start_pos = ()
                    else:
            ...
            for col_index, piece in enumerate(row):
                if piece in self.image_dict.keys():
                    ...
                    if piece == self.checked + "K":
                        pg.draw.rect(self.screen, DARKRED, pg.Rect(
